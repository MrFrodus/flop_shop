  ...
  return knex.schema
    ...
    .createTable("product_review", (table) => {
      ...
      table
        ...
        .withKeyName("fk_review_parent");
      table
        .foreign("product_id")
        .references("product.id")
        .withKeyName("fk_review_product");
    ...
    .createTable("category", (table) => {
      ...
      table
        ...
        .withKeyName("fk_category_parent");
    ...
    .createTable("product_category", (table) => {
      ...
      table
        .foreign("product_id")
        .references("product.id")
        .withKeyName("fk_pc_product");
      table
        .foreign("category_id")
        .references("category.id")
        .withKeyName("fk_pc_category");
      table.timestamp("created_at").notNullable().defaultTo(knex.fn.now());
      table
        .timestamp("updated_on")
        .nullable()
    ...
    .createTable("cart_item", (table) => {
      ...
      table
        .foreign("product_id")
        .references("product.id")
        .withKeyName("fk_cart_item_product");
      table
        .foreign("cart_id")
        .references("cart.id")
        .withKeyName("fk_cart_item_cart");
    ...
    .createTable("order_item", (table) => {
      ...
      table
        .foreign("product_id")
        .references("product.id")
        .withKeyName("fk_order_item_product");
      table
        .foreign("order_id")
        .references("order.id")
        .withKeyName("fk_order_item_order");
      table.index("order_id", "idx_order_item_order");
      table.index("product_id", "idx_order_item_product");
    })
    .createTable("shipping", (table) => {
      table.increments("id");
      table.integer("order_id").unsigned().unique().notNullable();
      table.integer("user_id").unsigned().notNullable();
      table.string("region", 50).nullable();
      ...
      table
        .enum("method", ["courier", "post_office", "self_delivery"])
        .notNullable();
      table.timestamp("created_at").notNullable().defaultTo(knex.fn.now());
      table
        .timestamp("updated_on")
      ...
      table
        .foreign("order_id")
        .references("order.id")
        .withKeyName("fk_shipping_order");
      table
        .foreign("user_id")
        .references("user.id")
        .withKeyName("fk_shipping_user");
      table.index("user_id", "idx_shipping_user");
      table.index("order_id", "idx_shipping_order");
    })
    .createTable("transaction", (table) => {
      ...
      table
        .foreign("user_id")
        .references("user.id")
        .withKeyName("fk_transaction_user");
      table
        .foreign("order_id")
        .references("order.id")
        .withKeyName("fk_transaction_order");
